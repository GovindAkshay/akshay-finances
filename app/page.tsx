...
export default function FinanceTracker() {
  ...
  const [newRent, setNewRent] = useState({date:'',description:'',amount:'',account:'PNC'});
  ...
  const applyFilter = (arr:any[]) => {
    let r = selectedMonth ? arr.filter(e=>e.date?.startsWith(selectedMonth)) : [...arr];
    if (search) r = r.filter(e=>JSON.stringify(e).toLowerCase().includes(search.toLowerCase()));
    return r.sort((a,b)=>(b.date||'').localeCompare(a.date||''));
  };

  const fIncome = applyFilter(incomeEntries);
  const fExpenses = applyFilter(expenseEntries);
  const fEmi = applyFilter(emiPayments);
  const fSips = applyFilter(sips);
  const fTransfers = applyFilter(transfers);
  const fRent = applyFilter(expenseEntries.filter(e=>e.category==='Rent'));
  ...
  const totalRent = sum(fRent);
  const totalBalance = Object.values(accountBalances).reduce((s:number,v:any)=>s+v,0) as number;

  const prevMonth = () => { const d=new Date((selectedMonth||new Date().toISOString().slice(0,7))+'-01'); d.setMonth(d.getMonth()-1); return d.toISOString().slice(0,7); };
  ...
  const addRent = async () => {
    if(!newRent.date||!newRent.amount) return;
    await addDoc(collection(db,'expenses'),{...newRent,category:'Rent',amount:parseFloat(newRent.amount)});
    setNewRent({date:'',description:'',amount:'',account:'PNC'}); setShowAddRent(false);
  };
  const addEmi = async () => {
  ...
  const sendEmail = async () => {
    ...
    try {
      await emailjs.send(EMAILJS_SERVICE,EMAILJS_TEMPLATE,{
        ...
        month:selectedMonth?monthName(selectedMonth):'All Time',
        income:usd(totalIncome)+'('+inr(totalIncome)+')',
        expenses:usd(totalExpenses)+'('+inr(totalExpenses)+')',
        balance:usd(totalIncome-totalExpenses),
        emi:inr(totalEMI),sip:inr(totalSIP),
  ...
  const generatePDF = async () => {
    ...
    pdf.text(`${selectedMonth?monthName(selectedMonth):'All Time'} · ${user?.email}`,pw/2,28,{align:'center'});
    ...
    (pdf as any).autoTable({startY:(pdf as any).lastAutoTable.finalY+8,head:[['','USD','INR']],body:[['Income',usd(totalIncome),inr(totalIncome)],['Expenses',usd(totalExpenses),inr(totalExpenses)],['Rent',usd(totalRent),inr(totalRent)],['Net',usd(totalIncome-totalExpenses),inr(totalIncome-totalExpenses)]],headStyles:{fillColor:[30,64,175]},theme:'grid'});
    ...
    pdf.save(`finances-${selectedMonth||'all'}.pdf`);
  };
  ...
  const Trend = ({curr,prev}:{curr:number,prev:number}) => {
    ...
    const up=p>=0;
    return <span style={{fontSize:11,fontWeight:600,color:up?'#ef4444':'#10b981',background:up?'#fef2f2':'#f0fdf4',padding:'2px 6px',borderRadius:4,marginLeft:6}}>{up?'↑':'↓'}{Math.abs(p)}% vs last mo</span>;
  };
  ...
  const AddForm = ({show,onClose,title,color,children,onAdd}:{show:boolean,onClose:()=>void,title:string,color:string,children:any,onAdd:()=>void}) => {
    ...
    return (
      <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.4)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
        <div style={{background:'white',borderRadius:20,padding:28,width:'100%',maxWidth:480,boxShadow:'0 20px 60px rgba(0,0,0,0.15)'}}>
  ...
  const Spinner = () => (
    <div style={{minHeight:'100vh',background:'#f8fafc',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{width:36,height:36,border:'3px solid #3b82f6',borderTopColor:'transparent',borderRadius:'50%',animation:'spin 0.8s linear infinite'}}></div>
    ...
    <div style={s.page}>
      <style>{`
        *{box-sizing:border-box}
        ...
        .tab-btn{background:transparent;border:none;border-bottom:2px solid transparent;color:#94a3b8;padding:12px 14px;cursor:pointer;font-size:13px;font-weight:500;transition:all 0.15s;white-space:nowrap;font-family:inherit}
        ...
        .hover-row:hover{background:#f1f5f9!important;transition:background 0.15s}
        .mob-nav{display:none}
        @media(max-width:768px){.mob-nav{display:flex}.desk-tabs{display:none}.desk-right{display:none}}
      `}</style>
      ...
      <div style={s.nav}>
        <div style={s.navInner}>
          ...
          <div className="desk-right" style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap' as const}}>
            ...
            <input placeholder="🔍 Search..." value={search} onChange={e=>setSearch(e.target.value)} style={{...s.input,width:150,padding:'6px 12px',fontSize:12,borderRadius:20}}/>
      ...
      {/* TABS */}
      <div style={s.tabBar} className="desk-tabs">
      ...
      <div style={s.main}>
        ...
        {activeTab==='dashboard'&&(
          <div className="fade">
            <div style={{background:'linear-gradient(135deg,#0f172a 0%,#1e3a8a 100%)',borderRadius:24,padding:'32px 36px',marginBottom:24,color:'white',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap' as const,gap:20}}>
              <div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px 28px'}}>
                <div>
                  <p style={{color:'#93c5fd',fontSize:10,letterSpacing:'0.08em',marginBottom:3}}>INCOME</p>
                  <p style={{color:'#4ade80',fontWeight:800,fontSize:20}}>{usd(totalIncome)}</p>
                  <Trend curr={totalIncome} prev={prevIncome}/>
                </div>
                <div>
                  <p style={{color:'#93c5fd',fontSize:10,letterSpacing:'0.08em',marginBottom:3}}>SPENT</p>
                  <p style={{color:'#f87171',fontWeight:800,fontSize:20}}>{usd(totalExpenses)}</p>
                  <Trend curr={totalExpenses} prev={prevExpenses}/>
                </div>
                <div>
                  <p style={{color:'#93c5fd',fontSize:10,letterSpacing:'0.08em',marginBottom:3}}>SAVINGS RATE</p>
                  <p style={{color:savingsRate>=20?'#4ade80':'#fbbf24',fontWeight:800,fontSize:20}}>{savingsRate}%</p>
                </div>
                <div>
                  <p style={{color:'#93c5fd',fontSize:10,letterSpacing:'0.08em',marginBottom:3}}>PROJECTED SPEND</p>
                  <p style={{color:'#fbbf24',fontWeight:800,fontSize:20}}>{usd(projectedSpend)}</p>
                </div>
              </div>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:14,marginBottom:28}}>
              {(['PNC','Chase Checking','Chase Savings'] as string[]).map(acc=>{
                ...
                return (
                  <div key={acc} style={{background:'white',border:`1px solid ${stale?'#fca5a5':'#e2e8f0'}`,borderRadius:16,padding:20}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                      ...
                      <button onClick={()=>{setEditingBalance(acc);setTempBalance(String(accountBalances[acc]));}} style={{background:'#f8fafc',border:'1px solid #e2e8f0',color:'#94a3b8',borderRadius:6,padding:'3px 10px',fontSize:11,cursor:'pointer'}}>Edit</button>
                    </div>
                    {editingBalance===acc?(
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
                        <input type="number" value={tempBalance} onChange={e=>setTempBalance(e.target.value)} style={{...s.input,fontSize:18,fontWeight:700}} onKeyDown={e=>e.key==='Enter'&&saveBalance(acc)} autoFocus/>
                        <button onClick={()=>saveBalance(acc)} style={{...s.btn,background:'#3b82f6',color:'white',padding:'10px 14px',whiteSpace:'nowrap' as const}}>Save</button>
                        <button onClick={()=>setEditingBalance(null)} style={{...s.btn,background:'#f1f5f9',color:'#64748b',padding:'10px 10px'}}>✕</button>
                      </div>
                    ):(
                      <>
            ...
            <p style={{fontSize:11,fontWeight:700,letterSpacing:'0.1em',color:'#94a3b8',marginBottom:12}}>
              {selectedMonth?monthName(selectedMonth).toUpperCase()+' SUMMARY':'ALL TIME SUMMARY'}
            </p>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:12,marginBottom:28}}>
              {[
                ...
                {label:'Rent Paid',value:usd(totalRent),color:'#ef4444'},
          ...
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                ...
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{fIncome.length} records · Total: <span style={{color:'#10b981',fontWeight:700}}>{usd(totalIncome)}</span> <span style={{color:'#94a3b8',fontSize:12}}>{inr(totalIncome)}</span></p>
              </div>
              <button onClick={()=>setShowAddIncome(true)} style={{...s.btn,background:'#0f172a',color:'white'}}>+ Add Income</button>
            </div>
            ...
            {fIncome.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No income entries yet.</div>}
          </div>
        )}
          ...
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                ...
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{fExpenses.length} records · Total: <span style={{color:'#ef4444',fontWeight:700}}>{usd(totalExpenses)}</span> <span style={{color:'#94a3b8',fontSize:12}}>{inr(totalExpenses)}</span></p>
              </div>
            ...
            {fExpenses.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No expenses yet.</div>}
          </div>
        )}
        ...
        {activeTab==='rent'&&(
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                <h2 style={{fontSize:20,fontWeight:800,color:'#0f172a'}}>🏠 Rent</h2>
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{fRent.length} entries · Total: <span style={{color:'#ef4444',fontWeight:700}}>{usd(totalRent)}</span></p>
              </div>
              <button onClick={()=>setShowAddRent(true)} style={{...s.btn,background:'#0f172a',color:'white'}}>+ Add Rent</button>
            </div>
            <AddForm show={showAddRent} onClose={()=>setShowAddRent(false)} title="Add Rent Payment" color="#ef4444" onAdd={addRent}>
              <input type="date" value={newRent.date} onChange={e=>setNewRent({...newRent,date:e.target.value})} style={s.input}/>
              <input placeholder="Description (e.g. June rent - Dorchester)" value={newRent.description} onChange={e=>setNewRent({...newRent,description:e.target.value})} style={s.input}/>
              <input type="number" placeholder="Amount ($)" value={newRent.amount} onChange={e=>setNewRent({...newRent,amount:e.target.value})} style={s.input}/>
              <select value={newRent.account} onChange={e=>setNewRent({...newRent,account:e.target.value})} style={s.input}>
                <option>PNC</option><option>Chase Checking</option><option>Chase Savings</option>
              </select>
            </AddForm>

            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:24}}>
              <div style={{background:'linear-gradient(135deg,#fef2f2,#fee2e2)',border:'1px solid #fecaca',borderRadius:16,padding:20}}>
                <p style={{fontSize:11,color:'#ef4444',fontWeight:600,letterSpacing:'0.08em',marginBottom:6}}>TOTAL RENT PAID (ALL TIME)</p>
                <p style={{fontSize:28,fontWeight:800,color:'#dc2626'}}>{usd(expenseEntries.filter(e=>e.category==='Rent').reduce((s,e)=>s+e.amount,0))}</p>
                <p style={{fontSize:12,color:'#ef4444',marginTop:4}}>{inr(expenseEntries.filter(e=>e.category==='Rent').reduce((s,e)=>s+e.amount,0))}</p>
              </div>
              <div style={{background:'linear-gradient(135deg,#fef2f2,#fee2e2)',border:'1px solid #fecaca',borderRadius:16,padding:20}}>
                <p style={{fontSize:11,color:'#ef4444',fontWeight:600,letterSpacing:'0.08em',marginBottom:6}}>TOTAL PAYMENTS</p>
                <p style={{fontSize:28,fontWeight:800,color:'#dc2626'}}>{expenseEntries.filter(e=>e.category==='Rent').length}</p>
                <p style={{fontSize:12,color:'#ef4444',marginTop:4}}>months of rent logged</p>
              </div>
            </div>
              ...
              <div key={e.id} className="hover-row" style={s.row}>
                <div style={{flex:1}}>
                  <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:3}}>
                    <span style={{fontSize:14,fontWeight:600,color:'#0f172a'}}>{e.description||'Rent'}</span>
                    {e.account&&<span style={{...s.badge,background:ACC_LIGHT[e.account]||'#f1f5f9',color:ACC_COLOR[e.account]||'#64748b'}}>{e.account}</span>}
                  </div>
                  <span style={{fontSize:12,color:'#94a3b8'}}>{e.date}</span>
                </div>
            ...
            {fRent.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No rent entries yet.</div>}
          </div>
        )}
        ...
        {activeTab==='transfers'&&(
          <div className="fade">
            ...
            {fTransfers.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No transfers yet.</div>}
          </div>
        )}
          ...
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                ...
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{selectedMonth?monthName(selectedMonth):'Select a month to see budget'}</p>
              </div>
              <button onClick={()=>setShowAddBudget(true)} style={{...s.btn,background:'#0f172a',color:'white'}}>+ Set Budget</button>
            </div>
            ...
            <div style={{display:'grid',gap:12}}>
              {budgets.filter(b=>!selectedMonth||b.month===selectedMonth).map(b=>{
                  ...
                  <div key={b.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:14,padding:20}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                      <div>
                        ...
                        {b.month&&<span style={{fontSize:12,color:'#94a3b8',marginLeft:8}}>{monthName(b.month)}</span>}
                      </div>
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
              ...
              {budgets.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No budgets set yet.</div>}
            </div>
          </div>
        )}
                ...
                return (
                  <div key={g.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:16,padding:22}}>
                    ...
                    <div style={{display:'flex',gap:6,marginBottom:16}}>
                      ...
                      <span style={{...s.badge,background:`${PRIORITY_COLOR[g.priority]}18`,color:PRIORITY_COLOR[g.priority]}}>{g.priority}</span>
                    </div>
                    ...
                    <div style={{background:'#f8fafc',borderRadius:8,padding:'10px 12px',fontSize:12,color:'#64748b'}}>
                      Remaining: <strong style={{color:'#0f172a'}}>{usd(rem)}</strong>
                      {months&&<span style={{marginLeft:12}}>· ~<strong style={{color:'#0f172a'}}>{months} months</strong> at {usd(g.monthly)}/mo</span>}
                    </div>
                  </div>
                );
              })}
              {goals.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No goals yet.</div>}
            </div>
          </div>
        )}
          ...
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                ...
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{fEmi.length} entries · Total: <span style={{color:'#8b5cf6',fontWeight:700}}>{inr(totalEMI)}</span></p>
              </div>
            ...
            {fEmi.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No EMI entries yet.</div>}
          </div>
        )}
          ...
          <div className="fade">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div>
                ...
                <p style={{fontSize:13,color:'#94a3b8',marginTop:2}}>{fSips.length} entries · Total: <span style={{color:'#3b82f6',fontWeight:700}}>{inr(totalSIP)}</span></p>
              </div>
            ...
            <AddForm show={showAddSip} onClose={()=>setShowAddSip(false)} title="Add SIP Investment" color="#3b82f6" onAdd={addSip}>
              ...
              <input placeholder="Fund name (e.g. Kotak Multicap Fund)" value={newSip.name} onChange={e=>setNewSip({...newSip,name:e.target.value})} style={s.input}/>
            ...
            {fSips.length===0&&<div style={{textAlign:'center' as const,padding:48,color:'#94a3b8',fontSize:14}}>No SIP entries yet.</div>}
          </div>
        )}
      ...
      {/* MOBILE BOTTOM NAV */}
      <div className="mob-nav" style={{position:'fixed',bottom:0,left:0,right:0,background:'white',borderTop:'1px solid #e2e8f0',display:'flex',justifyContent:'space-around',padding:'8px 0 16px',zIndex:100}}>
        {TABS.slice(0,5).map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{background:'none',border:'none',display:'flex',flexDirection:'column' as const,alignItems:'center',gap:3,padding:'4px 8px',cursor:'pointer',color:activeTab===t.id?'#3b82f6':'#94a3b8',fontFamily:'inherit'}}>
      ...
      <p style={{textAlign:'center' as const,color:'#e2e8f0',fontSize:10,letterSpacing:'0.1em',paddingBottom:8}}>
